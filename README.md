...
2. Create your `application-properties` file. Duplicate the provided [example file](src/main/resources/application.properties.example) file, rename it to `application-properties` and remove the comments except for `#---`. Fill out the fields above the `#---` with your database and other configuration details. **Never commit this file to source control, keep it safe!**
